// Horizontal Gaussian blur shader for glow effect
...
// Input texture (border render target)
...
// Fragment shader - applies horizontal Gaussian blur
@fragment
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    // Calculate horizontal texel offset (1 pixel in texture space)
    let texel_offset = vec2<f32>(1.0 / uniforms.tex_size.x, 0.0) * uniforms.blur_scale;

    var color = vec4<f32>(0.0);

    // Sample at 9 horizontal offsets: -4, -3, -2, -1, 0, +1, +2, +3, +4
